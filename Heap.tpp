...
class Heap {
    ...
    public:
        ...
        void insert(T element) {
            ...
            heapIndex index=tree.size()-1;
             while(index>1 && this->tree.at(index)<this->tree.at(getParentPosition(index))){
                std::swap(this->tree.at(index), this->tree.at(getParentPosition(index)));
        ...
        void remove(T value) {
            typename heap::iterator iterator=std::find(this->tree.begin(), this->tree.end(), value);
            if(iterator != this->tree.end()){
                this->tree.erase(iterator);
            }
